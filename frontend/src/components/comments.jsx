...
const Comments = ({ blogId }) => {
  ...
  const fetchComments = async () => {
    try {
      const response = await fetchData(`http://localhost:8000/api/v1/users/comments/${blogId}`, 'GET', 'Failed to fetch comments');
  ...
  const handleAddComment = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/v1/users/comments`, {
        method: 'POST',
